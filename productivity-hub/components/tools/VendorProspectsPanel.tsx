...
export default function VendorProspectsPanel({
    ...
    const [minInstallations, setMinInstallations] = useState("");
    const [maxInstallations, setMaxInstallations] = useState("");
    ...
    const fetchVendors = useCallback(async () => {
        ...
        try {
            ...
            if (minInstallations) params.set("minInstallations", minInstallations);
            if (maxInstallations) params.set("maxInstallations", maxInstallations);
    ...
    }, [isOpen, selectedState, selectedDistrict, searchQuery, minInstallations, maxInstallations, page]);
    ...
    useEffect(() => {
        setPage(1);
    }, [selectedState, selectedDistrict, searchQuery, minInstallations, maxInstallations]);
    ...
    const hasActiveFilters = selectedDistrict || searchQuery || minInstallations || maxInstallations;
                ...
                {showFilters && (
                    <div className="flex flex-wrap gap-3 p-4 bg-background rounded-xl border border-border/30">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-xs text-text-secondary">Installations:</span>
                            <input
                                type="number"
                                placeholder="Min"
                                value={minInstallations}
                                onChange={(e) => setMinInstallations(e.target.value)}
                                className="w-20 px-2 py-2 bg-background-card border border-border/50 rounded-lg text-sm focus:border-green/50 outline-none"
                            />
